...
class Pdf(FPDF):
    ...
    def mostrarDatos(self , solicitudes):
        ...
        self.cell(100 , 10, "Solicitudes", 1, 0, 'C')      
        ...
        for solicitud in solicitudes:
            self.cell(40 , 10, f"{solicitud.sol_usuario.first_name}{
                solicitud.sol_usuario.last_name}", 1,0, 'L')
            self.cell(100 , 10, solicitud.sol_descripcion, 1,0, 'L')
            self.cell(30 , 10, solicitud.sol_oficina_ambiente.ofi_nombre, 1,0, 'L')
            self.cell(20 , 10, str(solicitud.fecha_hora_creacion), 1,0, 'C')
            self.ln()
            fila += 4
        self.ln()
        self.set_font("Arial", "B", 12)
        self.cell(200,10 , '', 0,0,'C')
        self.ln()
